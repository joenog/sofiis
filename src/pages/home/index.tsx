...
export function Home() {
  ...
  const [visibleCont, setVisibleFiis] = useState(8);
        ...
        >
          ...
          <section className="flex !my-2 flex-col w-full md:w-6/10 gap-2">
            ...
            {data.slice(0, visibleCont).map((item, index) => (
              ...
              >
                ...
                <span>R$ {item.results[0]?.regularMarketPrice} </span>
                {
                  <span
                    className={
                      item.results[0]?.regularMarketChangePercent < 0
                        ? "text-red-600"
                        : "text-green-600"
                    }
                  >
                    {" "}
                    {item.results[0]?.regularMarketChangePercent.toFixed(2)}%
                  </span>
                }
              </div>
            ))}
            <span className="flex justify-end !mb-18">
              <button
                onClick={loadMore}
                className="!px-4 !py-2 transition bg-gray-800 rounded-xl hover:bg-slate-700"
                type="button"
              >
